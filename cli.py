...
@cli.command("start")
@click.option("--config", envvar="CALLISTO_CONFIG", type=click.Path(exists=True), help="config file")
@host_option
@port_option
...
def start_dev(config, host, port):
    ...
    p2 = run(["venv/bin/python", "-c", "from tests.utils.create_s3 import create_files; create_files()"])
    ...
    p3 = run(["venv/bin/python", __file__, "start"] + args, env={"FLASK_ENV": "development"})
    ps = [p0, p1, p3]
    try:
        while True:
